...
#define ZOOM 2
#define WIDTH (128 * ZOOM)
#define HEIGHT (88 * ZOOM)
...
void pceLCDTrans() {
	...
	for(y = 0; y < HEIGHT; y ++) {
		...
		for(x = 0; x < WIDTH; x ++) {
			...
			switch(c) {
			case 0:
				putPixelScreen(0x00ffffff, x, y);
				break;
			case 1:
				putPixelScreen(0x00aaaaaa, x, y);
				break;
			case 2:
				putPixelScreen(0x00555555, x, y);
				break;
			case 3:
				putPixelScreen(0, x, y);
				break;
			}
		}
	}
...
ScePspIVector2 mPos, mSpeed;
int mR;

int nGrain;

int _time = FRAMERATE * 60, score = 0, height = 0, dispscore = 0;
int hiScore[2] = {0, 0};
...
void pceAppInit(void)
{
	...
	FILE *fa = fopen("spout.sco", "rb");

	if(fa) {
		fread((void *)hiScore, 8, 1, fa);
		fclose(fa);
	}
...
void pceAppProc(int cnt)
{
	...
	sceCtrlReadBufferPositive(&pad, PSP_CTRL_MODE_ANALOG);
...
//				pceFileClose(&fa);
		...
		fnt_pos(0, 82);
		if(height > 0) {
			fnt_printf("time:%2d height:%4d score:%6d", (_time + FRAMERATE - 1) / FRAMERATE, height % 10000, score % 1000000);
		} else {
			fnt_printf("time:%2d height:   0 score:%6d", (_time + FRAMERATE - 1) / FRAMERATE, score % 1000000);
		}
		fnt_type(0);
	}

	if((pad.Buttons & PSP_CTRL_TRIANGLE) && gamePhase == 3 && gameover == 0) {
		fnt_type(2 + 128);
		fnt_pos(64 - 7 * 4 / 2, 33);
		fnt_printf(" pause ");
		fnt_type(0);
		...
		if(gameover == 0) {
			...
			if(mPos.y < 40 * 256) {
				...
				if(height > 0) {
					score ++;
					if((height & 127) == 0) {
						score += (_time + FRAMERATE - 1) / FRAMERATE * 10;
						_time += 60 * FRAMERATE;
						if(_time > 99 * FRAMERATE) {
							_time = 99 * FRAMERATE;
						}
						fnt_type(2 + 128);
						fnt_pos(4 * 5, 82);
						fnt_printf("%2d", (_time + FRAMERATE - 1) / FRAMERATE);
						fnt_type(0);
					}
					fnt_type(2 + 128);
					fnt_pos(4 * 15, 82);
					fnt_printf("%4d", height % 10000);
					fnt_type(0);
				}
			...
			if((upperLine & 31) == 0) {
				...
				switch(upperLine / 32) {
					...
					case 2:
						...
						fnt_type(2 + 128);
						fnt_pos(118 - 20 * 4, 0);
						fnt_printf("    height: %8d", hiScore[1] % 1000000);
						fnt_pos(118 - 20 * 4, 6);
						fnt_printf("high-score: %8d", hiScore[0] % 1000000);
						break;
...

	{
		...
		if(gamePhase & 2) {
			if(gameover == 0 && (pad.Buttons & PSP_CTRL_CROSS)) {
				int i, t, x, y;
				for(i = 0; i < 5; i ++) {
	...
	if((gamePhase & 2) && _time && gameover == 0) {
		_time --;
		if((_time % FRAMERATE) == 0) {
			fnt_type(2 + 128);
			fnt_pos(4 * 5, 82);
			fnt_printf("%2d", (_time + FRAMERATE - 1) / FRAMERATE);
			fnt_type(0);
	...
	if(dispscore < score) {
		...
		fnt_type(2 + 128);
		fnt_pos(4 * 26, 82);
		fnt_printf("%6d", dispscore % 1000000);
		fnt_type(0);
	}

	if(gamePhase == 3 && gameover != 0) {
		fnt_type(2 + 128);
		fnt_pos(64 - 11 * 4 / 2, 33);
		fnt_printf(" game over ");
		fnt_type(0);
	}
